...
// Following along with youtube - first part (until setup) copied from github repo.  
pragma solidity 0.8.24;

import {Test, console2} from "forge-std/Test.sol";
import {MyZkMinimalAccount} from "src/zksync/MyZkMinimalAccount.sol";
...
contract MyZkMinimalAccountTest is Test, ZkSyncChainChecker {
    using MessageHashUtils for bytes32;

    MyZkMinimalAccount minimalAccount;
    ERC20Mock usdc;
    ...
    function setUp() public {
        minimalAccount = new MyZkMinimalAccount();
        minimalAccount.transferOwnership(ANVIL_DEFAULT_ACCOUNT);
        usdc = new ERC20Mock();
        vm.deal(address(minimalAccount), AMOUNT);
    }

    function testZekOwnerCanExecuteCommands() public {
      address dest = address(usdc); 
      uint256 value = 0; 
      bytes memory functionData = abi.encodeWithSelector(ERC20Mock.mint.selector, address(minimalAccount), AMOUNT);

      // this is userOp in normal EVM chain. 
